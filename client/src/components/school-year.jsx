          ...
          {schoolYears.length > 0 ? (
            schoolYears.map((sy) => (
              <Button
                key={sy.schoolYearId}
                variant="outline"
                className="w-full text-lg font-semibold justify-start text-center px-4 py-6 cursor-pointer hover:bg-secondary hover:text-primary hover:shadow transition"
                onClick={() => onSelect?.(sy)}
              >
                {sy.schoolYearName}
              </Button>
